...
MAX_RETRIES = 20  # أقصى عدد للمحاولات
...
# ==================== قائمة المواقع الاحتياطية ====================
BACKUP_SITES = [
    "https://www.militadowatch.com",
    "https://newyes1.myshopify.com",
    "https://phomemo.com",
...
def process_single_attempt(ccx, site, task_id, attempt_number):
    """
    دالة معالجة بطاقة واحدة وموقع واحد - محاولة واحدة
    ...
    parts = ccx.split('|')
    if len(parts) != 4:
        return {"success": False, "code": None, "error": "Invalid card format", "task_id": task_id, "attempt": attempt_number}
    ...
    try:
        ...
        if r.status_code != 200:
            return {"success": False, "code": None, "error": f"Failed to fetch products (HTTP {r.status_code})", "task_id": task_id, "attempt": attempt_number}
        ...
        if not shippable_products:
            return {"success": False, "code": None, "error": "No shippable product found", "task_id": task_id, "attempt": attempt_number}
        ...
        if resp.status_code != 200:
            return {"success": False, "code": None, "error": f"Failed to add to cart (HTTP {resp.status_code})", "task_id": task_id, "attempt": attempt_number}

        response = s.post(f'{site}/cart', data={'checkout': ''}, proxies=proxies, timeout=100)
        checkout_url = response.url

    except Exception as e:
        return {"success": False, "code": None, "error": f"Request error: {str(e)}", "task_id": task_id, "attempt": attempt_number}
    ...
    try:
        ...
        try:
            ...
            return {"success": False, "code": None, "error": f"Shipping fill failed: {str(e)}", "task_id": task_id, "attempt": attempt_number}
        ...
        try:
            ...
            if not (card_filled and expiry_filled and cvv_filled and name_filled):
                return {"success": False, "code": None, "error": "Payment fill failed - fields not found", "task_id": task_id, "attempt": attempt_number}

        except Exception as e:
            return {"success": False, "code": None, "error": f"Payment error: {str(e)}", "task_id": task_id, "attempt": attempt_number}
        ...
        try:
            ...
            if driver:
                driver.quit()
            return {"success": False, "code": None, "error": f"GraphQL interception error: {str(e)}", "task_id": task_id, "attempt": attempt_number}
        ...
        if driver:
            try:
                ...
                pass
        return {"success": False, "code": None, "error": f"Browser error: {str(e)}", "task_id": task_id, "attempt": attempt_number}
...
def ff_with_retry(ccx, site, task_id):
    """
    دالة معالجة بطاقة مع نظام إعادة المحاولات التلقائي
    تعيد المحاولة حتى 20 مرة مع تغيير الموقع عند الفشل
    """
    logger.info(f"[{task_id}] Starting task with retry system for card: {ccx[:6]}... on site: {site}")
    ...
    # قائمة المواقع للتجربة (الموقع الأصلي + المواقع الاحتياطية)
    sites_to_try = [site] + BACKUP_SITES
    # إزالة التكرارات مع الحفاظ على الترتيب
    seen = set()
    sites_to_try = [x for x in sites_to_try if not (x in seen or seen.add(x))]

    for attempt_num in range(1, MAX_RETRIES + 1):
        # اختيار الموقع المناسب
        if attempt_num <= len(sites_to_try):
            current_site = sites_to_try[attempt_num - 1]
        else:
            # إذا خلصت كل المواقع، نعيد التدوير
            current_site = random.choice(sites_to_try)

        logger.info(f"[{task_id}] Attempt {attempt_num}/{MAX_RETRIES} on site: {current_site}")

        if task_id:
            with active_tasks_lock:
                if task_id in active_tasks:
                    active_tasks[task_id]["current_attempt"] = attempt_num
                    active_tasks[task_id]["current_site"] = current_site

        result = process_single_attempt(ccx, current_site, task_id, attempt_num)

        if task_id:
            with active_tasks_lock:
                if task_id in active_tasks:
                    active_tasks[task_id]["attempts"].append({
                        "attempt": attempt_num,
                        "site": current_site,
        ...
        # إذا نجحت العملية (حصلنا على كود) - نرجع النتيجة فوراً
        if result.get("success") and result.get("code"):
            logger.info(f"[{task_id}] SUCCESS on attempt {attempt_num}! Code: {result.get('code')} | Site: {current_site}")
            if task_id:
                with active_tasks_lock:
                    if task_id in active_tasks:
                        active_tasks[task_id]["status"] = "completed"
                        active_tasks[task_id]["result"] = result.get("code")
                        active_tasks[task_id]["final_attempt"] = attempt_num
            result["total_attempts"] = attempt_num
            result["final_site"] = current_site
            return result

        # إذا كان الخطأ يتعلق بالبطاقة نفسها (وليست مشكلة في الموقع) - نتوقف فوراً
        ...
        is_card_error = False
        for card_err in card_related_errors:
            if card_err.lower() in error_msg.lower():
                is_card_error = True
                break

        if is_card_error:
            logger.info(f"[{task_id}] Card error detected, stopping retries: {error_msg}")
            result["total_attempts"] = attempt_num
            return result

        # انتظار قصير بين المحاولات
        logger.info(f"[{task_id}] Attempt {attempt_num} failed: {error_msg}. Retrying with different site...")
        time.sleep(1)

    # إذا وصلنا إلى هنا، كل المحاولات فشلت
    logger.info(f"[{task_id}] All {MAX_RETRIES} attempts failed.")

    if task_id:
        with active_tasks_lock:
            if task_id in active_tasks:
                active_tasks[task_id]["status"] = "all_failed"
                active_tasks[task_id]["final_attempt"] = MAX_RETRIES
...
def home():
    ...
    # تنفيذ المهمة مع نظام إعادة المحاولات
    future = executor.submit(ff_with_retry, cc, url, task_id)

    try:
        result = future.result(timeout=REQUEST_TIMEOUT * MAX_RETRIES)
